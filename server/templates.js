...
_A theoretical framework or concept appearing in the corpus._
...
_${primaryType === 'Person' ? 'A person' :
   ...
   'An entity'} referenced in the corpus._
  ...
  if (options.grouped) {
    ...
    for (const letter of Object.keys(grouped).sort()) {
      ...
      for (const item of grouped[letter]) {
        ...
        itemsContent += `- ${link}${item.count ? ` (${item.count}${options.countLabel || ''})` : ''}\n`;
      }
    }
  } else {
    for (const item of items) {
...
Welcome to your AI-generated knowledge graph. This vault contains structured extractions from your documents, organized for exploration and connection.
...
### By Theme
${stats.topThemes?.length > 0 ? stats.topThemes.map(t => `- ${t}`).join('\n') : '_Themes not yet indexed_'}
